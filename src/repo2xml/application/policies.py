...
class ReasonFormatter:
    ...
    We keep this in the application layer (not in ingestor/scanner) so low-level
    components can return structured reasons without dictating UX strings.
    """
...
class SymlinkPolicy:
    ...
    def apply(self, entry: FileEntry) -> Optional[FilePayload]:
        ...
        - as-link: emit LinkPayload without reading content
        - skip: scanner usually skips symlink files, but we handle defensively
        """
        ...
        if entry.is_symlink and self.config.symlinks_files == SymlinkFilesMode.skip:
            # Normally symlink files are skipped by scanner, but keep this as a safety net.
...
class ModePolicy:
    ...
    def apply(self, entry: FileEntry) -> Optional[FilePayload]:
        """Mode policy: metadata mode emits metadata-only payload."""
...
class BinaryPolicy:
    ...
    def apply(self, entry: FileEntry) -> Optional[FilePayload]:
        """
        Binary policy: invoked after sniff() indicates binary.
        """
...
class TextPolicy:
    ...
    def apply(self, entry: FileEntry, *, encoding_hint: Optional[str]) -> FilePayload:
        """
        Text policy: read bounded text, apply processors, return TextPayload or skipped/error payload.
        """
        # Fast skip without extra IO when size is already known from scan.
...
class PayloadBuilder:
    """
    High-level policy coordinator.

    This isolates policy decisions from the pipeline's orchestration logic.
    """

    config: Repo2XMLConfig
    ingestor: IngestorLike

    # With slots=True, private fields must be declared explicitly.
    ...
    def build(self, entry: FileEntry) -> FilePayload:
        """
        Build a FilePayload for the given entry according to config policies.
        """
        # Symlink overrides content reads.
        p = self._symlink.apply(entry)
        if p is not None:
            return p

        # Mode-specific short-circuit.
        p = self._mode.apply(entry)
        if p is not None:
            return p

        # Full mode: sniff first.
